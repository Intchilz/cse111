'''
I have exceeded requirements. I added a function named
collect_element_name which gets the list created from
the parse_formula function and extracts the symbol. 
It then checks the periodic table using the symbol[key]
and returns the name of the element.
'''
...
def main():
    ...
    convert_formula = parse_formula(chemical_formula, periodic_table)
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    compute_mass = compute_molar_mass(convert_formula, periodic_table)
    # Compute the number of moles in the sample.
    compute_moles = mass/compute_mass
    # Print the molar mass.
    print(f"{compute_mass} grams/mole")
    # Print the number of moles.
    print(f"{round(compute_moles, 5)} moles")
    element = collect_element_name(convert_formula, periodic_table)
    print(f'You entered: {element}')

def make_periodic_table():
    '''Create a compound list
    which holds 94 lists of all
    the elements on a periodic table.'''
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    total_molar_mass = 0.0  # Initialize the total molar mass
    
    # Iterate over each element in the symbol_quantity_list
    for element in symbol_quantity_list:
        symbol = element[SYMBOL_INDEX]  # Extract the symbol
        quantity = element[QUANTITY_INDEX]  # Extract the quantity
        
        # Get the atomic mass from the periodic_table_dict
        atomic_mass = periodic_table_dict[symbol][ATOMIC_MASS_INDEX]
        
        # Calculate the contribution to the molar mass
        total_molar_mass += atomic_mass * quantity
    
    return total_molar_mass  # Return the total molar mass

def collect_element_name(symbol_quantity_list, periodic_table_dict):
    ...
    for element in symbol_quantity_list:
        ...
        name_from_key = periodic_table_dict[symbol][0]
        

        
        element_name = f"{element_name} ,{name_from_key }"
